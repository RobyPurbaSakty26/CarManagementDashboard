...
app.get('/', async (req, res) => {
  try {
    const users = await axios.get('http://localhost:9000/cars');
...
app.post('/add-car', uploadOnMemory.single('foto'), (req, res) => {
  ...
  cloudinary.uploader.upload(file, { folder: 'test' }, async function (err, result) {
    ...
    try {
      const users = await axios.post('http://localhost:9000/cars', body);
      return res.redirect('/');
    } catch (err) {
...
app.get('/update-car/:id', async (req, res) => {
  try {
    ...
    const cars = await axios.get(`http://localhost:9000/cars/${id}`);
...
app.post('/update-car/:id', uploadOnMemory.single('foto'), (req, res) => {
  ...
  cloudinary.uploader.upload(file, { folder: 'test' }, async function (err, result) {
    ...
    body.foto = result.url;
    try {
      const users = await axios.put(`http://localhost:9000/cars/${id}`, body);
      return res.redirect('/');
    } catch (err) {
      return res.status(500).json(err);
    }
...
app.get('/delete-cars/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const users = await axios.delete(`http://localhost:9000/cars/${id}`);
    res.redirect('/');
  } catch (err) {
    res.status(500).json(err);
  }
